  ...
  computed: {
    calcImageList () {
      ...
      if (this.configLimitList.length > 0) {
        let tempLimit =
            JSON.parse(JSON.stringify(this.configLimitList)).reduce((all, cur) => {
              if (cur.result.image) {
                all.push(cur);
              }
              return all;
    ...
    calcBwRange () {
      ...
      const bwArr = temp.reduce((all, cur) => {
        ...
        // dcim限制bw都是范围
        rangeArr = Array.from(new Set(cur.result.bw.reduce((sum, pre) => {
          sum.push(pre.min, pre.max);
          return sum;
        }, []))).sort((a, b) => a - b);
        min = rangeArr[0] === '' ? rangeArr[1] : rangeArr[0];
        max = rangeArr[rangeArr.length - 1] > maxBw ? maxBw : rangeArr[rangeArr.length - 1];
    ...
    calcFlowList () {
      ...
      if (temp.length > 0) {
        ...
        const flowArr = temp.reduce((all, cur) => {
          let rangeArr = Array.from(new Set(cur.result.flow.reduce((sum, pre) => {
            sum.push(pre.min, pre.max);
            return sum;
          }, []))).sort((a, b) => a - b);
          const min = rangeArr[0] === '' ? rangeArr[1] : rangeArr[0];
          const max = rangeArr[rangeArr.length - 1] > maxFlow[maxFlow.length - 1] ? maxFlow[maxFlow.length - 1] : rangeArr[rangeArr.length - 1];
        ...
        if (this.lineDetail.ip) {
          ...
          if (this.cloudData?.ip_num !== 0) {
            ...
            if (this.lineDetail.ip[0].type === 'radio') {
              ...
              this.params.ip_num = this.lineDetail.ip[0].min_value * 1 || temp_ip;
            }
          }
