<?php /*a:2:{s:74:"/www/wwwroot/cloud.hf.pw2/public/themes/clientarea/ogmiao/includes/pay.tpl";i:1719999923;s:73:"/www/wwwroot/cloud.hf.pw2/public/themes/clientarea/ogmiao/error/alert.tpl";i:1719999922;}*/ ?>
        ...
        <div class="pay-body">
            ...
            <h4 class="pay-title">
                <?php if($Action=='recharge'): ?>
                    充值账单
                <?php elseif($Pay['PayStatus'] !=='Paid'): ?>
                    总价<?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['total']; ?>，还需支付：
                    <!-- <?php if($Pay['use_credit_limit']): ?>
                        ，还需支付：
                    <?php else: if($Pay['deduction']): ?>
                            ，还需支付：
                        <?php endif; ?>
                    <?php endif; ?> -->
                <?php endif; ?>
            </h4>
              <?php if($Pay['pay_html']['type']=='url'): ?>
                <h2 style="margin-top:10px">
                    <?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['total']; ?><?php echo $ViewBilling['currency']['suffix']; ?>
                </h2>
             <?php endif; ?>
            <!-- <div class="pay-amount"><?php echo $Pay['total_desc']; ?></div> -->
            <?php if($Pay['use_credit_limit']): ?>
                <!-- <div class="btn-group">
                    <button type="button" class="btn btn-primary active">信用额支付</button>
                    <?php if($Pay['action'] == 'viewbilling'): ?>
                        <a href="/viewbilling?id=<?php echo $Pay['invoiceid']; ?>&use_credit_limit=0" class="btn btn-default active" role="button">现金支付</a>
                    <?php else: ?>
                        <a href="javascript: payamount(<?php echo $Pay['invoiceid']; ?>,0);" class="btn btn-default active" role="button">现金支付</a>
                    <?php endif; ?>
                </div> -->
                <div class="pay-area">
                    <div id="pay-type">
                        ...
                        <?php elseif($Pay['pay_html']['type']=='insert'&&$Pay['PayStatus']!='Paid'): ?>
                            <object data="<?php echo $Pay['pay_html']['data']; ?>" style="width: 200px; height: 210px;"></object>
                        <?php elseif($Pay['pay_html']['type']=='jump'): ?>
                            <div class="pay-text"><p>请在新页面进行支付<br/>如跳转失败，请点击<a href="<?php echo $Pay['pay_html']['data']; ?>">去支付</a></p></div>
                            <div class="pay-tools" style="margin-top:10px">
                                <select class="form-control select-pay" >
                                    <?php foreach($Pay['gateway_list'] as $item): ?>
                                        <option value="<?php echo $item['name']; ?>" <?php if($item['name']==$Pay['payment']): ?>selected<?php endif; ?>><?php echo $item['title']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        <?php elseif($Pay['pay_html']['type']=='html'): ?>
                            
                            <!--<?php echo $Pay['pay_html']['data']; ?>-->
                    ...
                    <div class="par_balance">
                        ...
                        <span class="balance_money" style="color:#1676FE"> <?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['total']; ?><?php echo $ViewBilling['currency']['suffix']; ?></span>
                    </div>
                    <?php endif; ?>
                    <!-- 支付成功遮罩层 -->
            ...
            <?php else: ?>
                <div class="pay-area">
                    ...
                    <?php if($Pay['PayStatus']!='Paid'): if($Pay['use_credit']==1&&$Pay['credit_enough']==1): ?>
                            <div class="par_balance">
                                <img src="/themes/clientarea/default/assets/images/xianjin-bg.png" alt="">
                                <span class="balance_money"> <?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['total']; ?><?php echo $ViewBilling['currency']['suffix']; ?></span>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
        ...
        <div class="modal-footer mt-2 d-flex justify-content-between ">
            <div class="d-flex align-items-center footer-pay-balance ">
                ...
                <span class="font-weight-bold mr-2">
                   可用信用额 <?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['credit_limit_balance']; ?>
                </span>
                <!-- <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="use_credit_limit" class="custom-control-input use_credit_limit" id="customCheck1" <?php if($Pay['use_credit_limit']==1): ?>checked<?php endif; ?> disabled>
                    <label class="custom-control-label" for="customCheck1">使用信用额支付</label>
                </div> -->
            </div>
            <div>
                <?php if($Pay['use_credit_limit']==1 || $Pay['pay']==true): ?>
    ...
    <?php else: ?>
        <div class="modal-footer mt-2 d-flex justify-content-between ">
            <div class="d-flex align-items-center footer-pay-balance ">
                ...
                <?php if($Action!='recharge'): if($Pay['credit']>0): ?>
                        <span class="mr-2">
                    <img width="20" src="/themes/clientarea/default/assets/images/gold.svg" alt="">
                </span>
                        <span class="font-weight-bold mr-2">
                    当前余额<?php echo $ViewBilling['currency']['prefix']; ?><?php echo $Pay['credit']; ?>
                </span>
                        <div class="custom-control custom-checkbox">
                            <input type="checkbox" name="use_credit" class="custom-control-input use_credit" id="customCheckPay" <?php if($Pay['use_credit']==1): ?>checked<?php endif; ?>>
            ...
            <div>
                ...
                <?php if(($Pay['use_credit']==1&&$Pay['credit_enough']==1) || $Pay['pay']==true): ?>
...
<script type="text/javascript" src="/themes/clientarea/default/assets/libs/qrcode/qrcode.min.js?v=<?php echo $Ver; ?>"></script>
<script type="text/javascript" src="/themes/clientarea/default/assets/libs/qrcode/jquery.qrcode.min.js?v=<?php echo $Ver; ?>"></script>

<script>
    var timer = null
    $(function(){
        let payStatus = '<?php echo $ViewBilling['detail']['status']; ?>';
        let orderNum = '<?php echo $ViewBilling['detail']['id']; ?>';
        let invoiceid = '<?php echo $Pay['invoiceid']; ?>';
        if(invoiceid == "" || invoiceid == null || invoiceid == undefined){
            invoiceid = '<?php echo $Post['invoiceid']; ?>';
        }
        $("#myLargeModalLabel").html("账单 - "+invoiceid);
        // 动态添加支付方式下拉
        if($(".modal-header .paySelect").length == 0 && payStatus!='Paid'){
            let pmAction = '<?php echo $Action; ?>';
            let is_open_credit_limit = '<?php echo $paymt['is_open_credit_limit']; ?>';
            let credit_limit_balance = '<?php echo $paymt['credit_limit_balance']; ?>';
            let is_open_shd_credit_limit = '<?php echo $paymt['is_open_shd_credit_limit']; ?>';
            let subtotal = '<?php echo $paymt['subtotal']; ?>';
            let invoiceid = '<?php echo $Pay['invoiceid']; ?>';
            let xyeHtml = '';
            let selectChecked = '<?php echo $Pay['use_credit_limit']; ?>'
            if(selectChecked==1){
        ...
        var pay_html = '<?php echo $Pay['pay_html']; ?>'
        var patStatus = '<?php echo $Pay['PayStatus']; ?>'
        if(patStatus=='Paid'){
        ...
        }else if('<?php echo $Pay['credit_enough']; ?>'==1&&'<?php echo $Pay['use_credit']; ?>'==1||!pay_html){
            let id = setTimeout(() => {  }, 0);
            while(id > 0){
        ...
        function init(){
            var pay = '<?php echo json_encode($Pay['pay_html']); ?>';
        ...
        function getData(){
            ...
            $.ajax({
                type: "POST",
                data: { invoiceid: '<?php echo $Pay['invoiceid']; ?>', use_credit:use_credit?1:0,payment: payment, use_credit_limit:0},
                url: url,
                success: function (data) {
    ...
    function payNow(){
        ...
        $.ajax({
            type: "POST",
            data: { invoiceid: '<?php echo $Pay['invoiceid']; ?>', use_credit:selectType == 0 ?1:0,payment: payment,use_credit_limit:selectType == 1 ?1:0},
            url: url,
            success: function (data) {
                clearInterval(timer)
                $("#pay-content,#pay .modal-body").html(data);
                ...
                if ('<?php echo $Pay['credit_enough']; ?>' == 1 || '<?php echo $Pay['use_credit_limit']; ?>') { // 余额足够 或者 使用信用额
                    if ('<?php echo $ReturnUrl; ?>') {
