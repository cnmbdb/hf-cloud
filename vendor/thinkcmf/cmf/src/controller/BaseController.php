...
class BaseController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $isLocalDevHost = preg_match('/^(127\.0\.0\.1|localhost)(:\d+)?$/', $host);
        if (cmf_is_installed() && !$isLocalDevHost){
            ...
            if(empty($zjmf_authorize)){
                ...
                if(time()>$auth['last_license_time']+7*24*3600
                    || (ltrim(str_replace('https://','',str_replace('http://','',$auth['domain'])),'www.')!=ltrim(str_replace('https://','',str_replace('http://','',$_SERVER['HTTP_HOST'])),'www.') && php_sapi_name()!='cli')
                    || $auth['installation_path']!=CMF_ROOT
                    || $auth['license']!=configuration('system_license')){
